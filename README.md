...
- **[`apps/data_cleaning/`](apps/data_cleaning/)** — rebuilds
  ...
  (B2B contact/company CSV data cleaning). Five typed tools: two pipeline
  tools (`clean_contacts`, `clean_accounts`) plus three small utility tools
  (`clean_email`, `normalize_country`, `extract_domain`) — the "many small
  typed tools" shape zeocore's own README pitches.
...
Each app is its own installable package, depending on the real, published
`zeocore` package from PyPI (`zeocore>=0.1.0` — not a local/editable
...
```bash
cd apps/data_cleaning && pip install -e .
cd apps/metrics_tracker && pip install -e .
```
...
```bash
cd apps/data_cleaning && python run_demo.py
cd apps/metrics_tracker && python run_demo.py
```
